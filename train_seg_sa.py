...
from sa_model import SASegStereoCNN
from tqdm import tqdm
from torchsummary import summary
import argparse
from dataset import StereoSegmentationDataset
...
if args.finetune and os.path.exists('best_stereo_cnn_sa_seg.pth'):
    print("loading existing model for finetune...")
    model.load_state_dict(torch.load('best_stereo_cnn_sa_seg.pth'))
...
if not args.finetune and os.path.exists('stereo_sa_seg_checkpoint.pth'):
    print("loading existing checkpoint ...")
    checkpoint = torch.load('stereo_sa_seg_checkpoint.pth')
    model.load_state_dict(checkpoint['model'])
    optimizer.load_state_dict(checkpoint['optimizer'])
    start_epoch = checkpoint['epoch']

if args.wandb:
    wandb.init(project='stereo-cnn', name='sa-segmentation', config=args)

best_loss = 100000
best_gd = 100000
best_epoch = 0
with open('ard_curves_sa_segmentation.txt', 'w') as f:
    ...
    for epoch in range(num_epochs):
        ...
        if total_test_loss < best_loss:
            ...
            torch.save(model.state_dict(), 'best_stereo_cnn_sa_seg.pth')
        ...
        torch.save({"epoch": epoch, "model": model.state_dict(), "optimizer": optimizer.state_dict()}, f'stereo_cnn_sa_seg_checkpoint.pth')
        torch.save(model.state_dict(), f'stereo_cnn_sa_seg_checkpoint_{epoch}.pth')
