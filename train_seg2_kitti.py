...
from sa_model import BaselineStereoCNN, SegStereoCNN2, SASegStereoCNN
from tqdm import tqdm
from torchsummary import summary
import argparse
from dataset_kitti import StereoDataset, StereoSegmentationDataset
...
model = SegStereoCNN2(device)
model.to(device)

summary(model, (9, 400, 879))

if args.finetune and os.path.exists('best_stereo_cnn_seg2.pth'):
    print("loading existing model for finetune...")
    model.load_state_dict(torch.load('best_stereo_cnn_seg2.pth'))
...
def evaluate(model, dataloader):
    ...
    with torch.no_grad():
        ...
        total_ard_values = 0
        total_gd = 0
        for left_image, right_image, left_mask, disparity_map, depth_map, focal_length, baseline in tqdm(dataloader):
...
if not args.finetune and os.path.exists('stereo_cnn_seg2_checkpoint.pth'):
    print("loading existing checkpoint ...")
    checkpoint = torch.load('stereo_cnn_seg2_checkpoint.pth')
    model.load_state_dict(checkpoint['model'])
    optimizer.load_state_dict(checkpoint['optimizer'])
    start_epoch = checkpoint['epoch']

if args.wandb:
    wandb.init(project='stereo-cnn', name='segmentation2', config=args)

best_loss = 100000
best_gd = 100000
best_epoch = 0
with open('ard_curves_segmentation2.txt', 'w') as f:
    ...
    for epoch in range(num_epochs):
        ...
        if total_test_loss < best_loss:
            ...
            torch.save(model.state_dict(), 'best_stereo_cnn_seg2.pth')
        ...
        torch.save({"epoch": epoch, "model": model.state_dict(), "optimizer": optimizer.state_dict()}, f'stereo_cnn_seg2_checkpoint.pth')
        torch.save(model.state_dict(), f'stereo_cnn_seg2_checkpoint_{epoch}.pth')
