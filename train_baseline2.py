...
from sa_model import BaselineStereoCNN, SegStereoCNN, SASegStereoCNN, BaselineStereoCNN2
...
model = BaselineStereoCNN2(device)
model.to(device)

summary(model, (6, 400, 879))


if args.finetune and os.path.exists('best_stereo_cnn_baseline.pth'):
    print("loading existing model for finetune...")
    model.load_state_dict(torch.load('best_stereo_cnn_baseline.pth'))
...
if not args.finetune and os.path.exists('stereo_cnn_baseline_checkpoint.pth'):
    print("loading existing checkpoint ...")
    checkpoint = torch.load('stereo_cnn_baseline_checkpoint.pth')
...
if args.wandb:
    wandb.init(project='stereo-cnn', name='baseline', config=args)

best_loss = 100000
best_gd = 100000
best_epoch = 0

with open('ard_curves_baseline2.txt', 'w') as f:
    ...
    for epoch in range(num_epochs):
        ...
        if total_test_loss < best_loss:
            ...
            torch.save(model.state_dict(), 'best_stereo_cnn_baseline.pth')
        ...
        torch.save({"epoch": epoch, "model": model.state_dict(), "optimizer": optimizer.state_dict()}, f'stereo_cnn_baseline_checkpoint.pth')
        torch.save(model.state_dict(), f'stereo_cnn_baseline_checkpoint_{epoch}.pth')
